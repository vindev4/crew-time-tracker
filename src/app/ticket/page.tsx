    ...
    <div className="min-h-screen p-4">
      <div className="max-w-lg mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-gray-800 rounded-xl p-6">
          ...
          <div className="mb-4">
            <label className="block text-sm text-gray-400 mb-2">Work Notes / Description</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={6} placeholder="Describe the work performed, any issues, materials used, etc." className="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 resize-none" disabled={loading} />
          </div>
          <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 disabled:opacity-50 text-white font-bold py-3 rounded-lg transition-colors">{loading ? "Submitting..." : "Submit Ticket"}</button>
        </form>
      </div>
    </div>
  );
